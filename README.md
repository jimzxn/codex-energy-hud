...
Windows 原生桌面悬浮挂件：Codex 剩余额度与剩余 Token 估算、任务活动概况与本轮时长、任务 Token 明细、CPU/GPU 占用、磁盘读写速率。当前版本 **1.4.0**。
...
同一次请求不会反复提醒；启动时不补报历史完成，重新连接也不会重复已知请求。窗口隐藏到托盘后仍可提醒。当前版本面向本机顶层任务，具体实时覆盖数量可在任务面板悬停查看。
