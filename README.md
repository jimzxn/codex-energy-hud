...
| **macOS · 源码实现** | Avalonia 界面与共享采集层；已在 Windows 主机完成编译兼容检查，尚未在 Mac 构建发布包、运行或验收。尚无实时用量曲线和费用报告 UI，GPU/显存及桌面实时审批/输入状态暂不可用。详见 [macOS 接手说明](docs/macos.md) |
...
每条曲线只在内存保留最近 60 秒，退出后不保存。隐藏到托盘后仍按秒采集，停止曲线绘制，恢复显示时呈现最新历史。本功能用于 Windows WPF 界面，macOS 源码实现尚未接入实时用量折线。
...
此金额是 **API Token 价格等值估算，不是订阅实际扣款或账单**；未计工具调用费、区域处理加价、税费及不可见的云端或其他设备用量。价格变化后需更新内置表，无法用当前表还原历史账单。费用 UI 仅接入 Windows，macOS 保留共享层编译兼容，尚无费用界面或 Mac 运行验收。
...
`--live-workload` 按秒只读采集实时用量，默认运行 600 秒；`--output` 保存 JSON 回执。结果包含有效时长、采样次数、每次读取耗时、待补读文件数、历史长度，以及测试进程的 CPU 和内存。`passed` 仅表示达到指定时长、没有完全不可用样本且整体历史不超过 60 点；是否存在持续积压、部分数据或内存增长仍须查看样本序列与读取耗时，不能只凭该布尔值认定全部验收完成。
