...
Windows 原生桌面悬浮挂件：Codex 剩余额度与剩余 Token 估算、任务活动概况与本轮时长、任务 Token 明细、CPU/GPU 占用、磁盘读写速率。当前版本 **1.4.2**。
...
采样中断、字段不完整或 Token 未匹配时，只丢弃尚未完成的当前段，保留此前闭合的有效分段；恢复后须完成一段新的有效变化才能重新显示估算，避免跨断档配对。账户切换、额度回升或周期重置仍会清空旧比例。展开面板显示已保留分段、累计下降、当前段进度，以及暂停或重新对齐原因。所选额度消失时仍须手动重新选择。
...
匿名校准历史保存在同一设置目录的 `token-estimates-*.json`，保存哈希标识、有效分段的时间和数值、最近额度读数，以及受控的采样状态和调整原因，不保存邮箱、账户 ID 原值、任务正文或登录令牌。历史分段须匹配账户与额度周期；重启后重新建立计数基线，取得有效新分段后才恢复估算，不跨停机时段计算增量。1.4.1 使用第 2 版校准格式，旧格式不用于新的估算，以隔离内部审批记录造成的旧口径影响。
