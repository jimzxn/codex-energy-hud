...
`QuotaTokenEstimator` 将账户、额度周期、额度回升的硬重置，与采样中断的暂停分开。暂停保留闭合分段和时间水位，舍弃未完成段，恢复后须用全新同步基线完成至少 2 个百分点的新段才能确认历史。模型与 Token 结构变化只标记用法波动，不破坏数值账本。

持久化第 2 版同时保存最后额度值与时间，防止重启后借用额度回升前的样本；旧口径历史不用于恢复。当前段进度及最近调整原因提供给 UI，后台也保存受控诊断文字，避免“校准中”而无法知道阻塞原因。
